...
for pattern in patterns:
    for img_path in folder_path.glob(pattern):
        try:
            ...
            # Open the image to check dimensions and file size
            file_size_kb = os.path.getsize(img_path) / 1024
            with Image.open(img_path) as img:
                width, height = img.size
                print(f"File: {img_path.name} | Width: {width}, Height: {height} | Size: {file_size_kb:.2f} KB")
                if width > 1500 or height > 1500 or file_size_kb > 100:
